...
1. **Voice notes.** Tap the microphone in the Notes capture bar, press `v` on a
   ...
   Quick capture widget. Recording starts at once; tap the stop button to save.
   A new note opens with the recording and a **Transcript** that fills in when
   transcription finishes. The audio stays in the note. Play, seek, transcribe
...
2. **Transcription settings** (Settings → Voice note transcription):
   - **On device** (default): Whisper runs inside OurNet. Audio never leaves the
     device. The first transcription asks to download a model once: *Accurate*
     (60 MB) or *Fast* (32 MB), fetched from the whisper.cpp model repository and
     checked against a pinned SHA-256 before use.
   - **System service** (Android 13 and later, opt-in with a warning): the
     phone's speech service, which may send audio to its provider.
